  ...
  return new ImageResponse(
    ...
    ),
    {
      ...
      headers: {
         ...
         * لینک شده) و بعد به‌عنوان «Crawled - currently not indexed» ثبتش
         * می‌کرد — هر ده نمونه‌ی آن دسته در سرچ‌کنسول همین مسیر بودند.
         *
         * noindex فقط جلوی *فهرست‌شدن* را می‌گیرد، نه دریافت را: خزنده‌های
         * تلگرام/واتس‌اپ/توییتر این هدر را اصلاً نمی‌خوانند و پیش‌نمایشِ
         * اشتراک‌گذاری دست‌نخورده می‌ماند. برای گوگل هم تصویر همچنان قابل
         * دریافت است؛ فقط خودش نتیجه‌ی جست‌وجو نمی‌شود — که همان چیزی است
         * که می‌خواهیم، چون این کارتِ ۱۲۰۰×۶۳۰ محتوای صفحه نیست.
         */
        "X-Robots-Tag": "noindex",
        /**
         * پیش‌فرضِ Next این‌جا `max-age=0, must-revalidate` بود، یعنی satori
         * برای هر بازدیدِ خزنده یک PNG کامل را از نو می‌ساخت. یک ساعت کش،
         * این هزینه را برمی‌دارد و بدترین حالتش این است که تا یک ساعت پس از
         * ویرایشِ کپشن، کارتِ قدیمی نشان داده شود.
         */
